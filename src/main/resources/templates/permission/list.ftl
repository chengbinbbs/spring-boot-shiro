	...
	<body>
		...
		<script>
			...
			layui.use(['paging', 'form'], function() {
                ...
                paging.init({
                    openWait: true,
					url: '/permission/list?v=' + new Date().getTime(), //地址
					elem: '#content', //内容容器
					params: { //发送到服务端的参数
					},
					type: 'POST',
                ...
                function viewForm(id) {
                    ...
                    $.get('/permission/view?permissionId=' + id, null, function(form) {
                ...
                function delForm(id) {
                    ...
                    layer.alert('',{
                        ...
                        $.ajax({
                            type: "GET",
                            url: "/permission/del?permissionId=" + id,
				...
				function editForm(id) {
                    //本表单通过ajax加载 --以模板的形式，当然你也可以直接写在页面上读取
                    id = id == undefined ? '':id;
                    $.get('/permission/edit?permissionId=' + id, null, function(form) {
                        addBoxIndex = layer.open({
                            type: 1,
                            title: '保存权限',
                            content: form,
