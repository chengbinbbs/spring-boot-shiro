	...
	<body>
		...
		<script type="text/html" id="tpl">
			...
			<tr>
                ...
                <td>{{ item.password }}</td>
                <td>
                    {{# layui.each(item.roles, function(index, role){ }}
                    <span class="layui-badge layui-bg-green">{{ role.role }}</span>
                    &nbsp;&nbsp;
                    {{# }); }}
                </td>
				<td>
                    {{# if(item.locked == false){ }} <span style="color: green">有效</span> {{#  } }}
                    {{# if(item.locked == true){ }} <span style="color: red">无效</span> {{#  } }}
				</td>
				<td>
		...
		<script>
			...
			layui.use(['paging', 'form'], function() {
                ...
                paging.init({
                    openWait: true,
					url: '/user/list?v=' + new Date().getTime(), //地址
					elem: '#content', //内容容器
					params: { //发送到服务端的参数
					},
					type: 'POST',
                ...
                function viewForm(id) {
                    ...
                    $.get('/user/view?userId=' + id, null, function(form) {
				...
				function editForm(id) {
                    //本表单通过ajax加载 --以模板的形式，当然你也可以直接写在页面上读取
                    id = id == undefined ? '':id;
                    $.get('/user/edit?userId=' + id, null, function(form) {
                        addBoxIndex = layer.open({
                            type: 1,
                            title: '保存用户',
                            content: form,
