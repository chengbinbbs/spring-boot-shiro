	...
	<body>
		<div class="admin-main">
            <blockquote class="layui-elem-quote">
                ...
                <button type="button" class="layui-btn layui-btn-small" id="edit"><i class="fa fa-edit" aria-hidden="true"></i> 编辑</button>
                <button type="button" class="layui-btn layui-btn-small" id="edit"><i class="fa fa-minus" aria-hidden="true"></i> 删除</button>-->
		...
		<script type="text/html" id="tpl">
			...
			<tr>
                ...
                <td>
                    {{# layui.each(item.permissions, function(index, permission){ }}
                    {{ permission.permission }}&nbsp;&nbsp;
                    {{# }); }}
                </td>
				<td>
                    {{# if(item.available == true){ }} <span style="color: green">有效</span> {{#  } }}
                    {{# if(item.available == false){ }} <span style="color: red">无效</span> {{#  } }}
				</td>
				<td>
		...
		<script>
			...
			layui.use(['paging', 'form'], function() {
                ...
                function viewForm(id) {
                    ...
                    $.get('/role/view?roleId=' + id, null, function(form) {
				...
				function editForm(id) {
                    //本表单通过ajax加载 --以模板的形式，当然你也可以直接写在页面上读取
                    id = id == undefined ? '':id;
                    $.get('/role/edit?roleId=' + id, null, function(form) {
                        addBoxIndex = layer.open({
                            type: 1,
                            title: '保存角色',
                            content: form,
